...
def main(args=None):
    ...
    while epoch < max_epochs:  # train until max_epochs is reached
        ...
        if (
            ...
            latest_scan, distance, cos, sin, collision, goal, a, reward, vel, pose = ros.reset()
            # 重置环境时清空雷达数据缓冲区
            scan_buffer.clear()
            # 将初始雷达数据添加到缓冲区
            scan_buffer.append(latest_scan)
...
def eval(model, env, scenarios, epoch, max_steps, state_dim, history_n, best_success, best_reward, linear_vel_scale, angular_vel_scale):
    ...
    for scenario in scenarios:
        count = 0
        latest_scan, distance, cos, sin, collision, goal, a, reward, vel, pose = env.eval(
            scenario=scenario
        )
        ...
        while count < max_steps:
            ...
            latest_scan, distance, cos, sin, collision, goal, a, reward, vel, pose = env.step(
